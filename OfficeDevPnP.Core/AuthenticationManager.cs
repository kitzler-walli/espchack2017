    ...
    public class AuthenticationManager
    {
        ...
        private async Task<AuthenticationResult> AcquireNativeApplicationTokenAsync(string authContextUrl, string resourceId)
        {
            ...
            if (ar == null)
            {
                try
                {
                    ar = _authContext.AcquireTokenAsync(resourceId, _clientId, _redirectUri, null).GetAwaiter().GetResult();//PromptBehavior.Always
        ...
        public ClientContext GetAzureADAppOnlyAuthenticatedContext(string siteUrl, string clientId, string tenant, X509Certificate2 certificate, AzureEnvironment environment = AzureEnvironment.Production)
        {
            ...
            clientContext.ExecutingWebRequest += (sender, args) =>
            {
                var ar = authContext.AcquireTokenAsync(host.Scheme + "://" + host.Host + "/", clientAssertionCertificate);
                args.WebRequestExecutor.RequestHeaders["Authorization"] = "Bearer " + ar.GetAwaiter().GetResult().AccessToken;
            };
